...
summary_data = [
    {"Indicator": "Farmgate Price {currency}", "Value": f"{farmgate_price*exchange_rate:,.2f} {currency}"},
    ...
    {"Indicator": "Break-Even Price {currency}", "Value": f"{required_price_to_break_even:,.2f} {currency}" if required_price_to_break_even is not None else "N/A"},
    {"Indicator": "Gross Margin {currency}", "Value": f"{gross_margin:,.2f}" if best_case_quantity is not None else "N/A"},
